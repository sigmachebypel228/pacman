...
# Настройка частоты появления новых точек
SPAWN_EVENT = pygame.USEREVENT + 1
pygame.time.set_timer(SPAWN_EVENT, 1000)  # Создание новой точки каждую секунду
...
def main():
    ...
    score = 0
    timer = 20  # Игровое время — 20 секунд
    ...
    while running:
        for event in pygame.event.get():
            ...
            # Появляется новая точка каждую секунду
        ...
        # Положение таймера и проверка игрового времени
        current_time = time.time()
        elapsed_time = current_time - start_time
        remaining_time = max(0, timer - elapsed_time)

        # Окончание игры через 20 секунд
        ...
        # Отображение таймера
        timer_text = font.render(f'Time Left: {remaining_time:.2f}s', True, WHITE)
    ...
    if game_over:
        ...
        game_over_text = font.render('Time\'s Up!', True, RED)
        score_text = font.render(f'Final Score: {score}', True, WHITE)
        SCREEN.blit(game_over_text, (WIDTH // 2 - game_over_text.get_width() // 2, HEIGHT // 2 - 100))
        SCREEN.blit(score_text, (WIDTH // 2 - score_text.get_width() // 2, HEIGHT // 2 + 50))
        pygame.display.flip()
        pygame.time.wait(5000)  # Пауза перед закрытием окна
